"""
Management command to set up Maruti workflow and seed sample data.
"""

from django.core.management.base import BaseCommand

from enrollment.models import WorkflowDefinition, WorkflowStep, WorkflowSequence


class Command(BaseCommand):
    help = 'Set up the Maruti vehicle enrollment workflow'

    def add_arguments(self, parser):
        parser.add_argument(
            '--clear',
            action='store_true',
            help='Clear existing Maruti workflow before creating'
        )

    def handle(self, *args, **options):
        if options['clear']:
            self.stdout.write('Clearing existing Maruti workflow...')
            WorkflowDefinition.objects.filter(brand='maruti').delete()

        # Create Maruti workflow
        self.stdout.write('Creating Maruti workflow...')

        workflow, created = WorkflowDefinition.objects.get_or_create(
            brand='maruti',
            defaults={
                'name': 'Maruti Suzuki Vehicle Enrollment',
                'description': 'Standard enrollment workflow for Maruti Suzuki vehicles',
                'is_active': True
            }
        )

        if not created:
            self.stdout.write(self.style.WARNING('Maruti workflow already exists, updating...'))

        # Define steps
        steps_data = [
            {
                'name': 'validate_vin',
                'step_type': 'sync',
                'order': 0,
                'api_endpoint': '/api/oem/maruti/validate-vin',
                'capabilities': [],
                'products': [],
                'timeout_seconds': 30,
                'retryable': True,
            },
            {
                'name': 'check_capabilities',
                'step_type': 'sync',
                'order': 1,
                'api_endpoint': '/api/oem/maruti/capabilities',
                'capabilities': ['gps', 'obd', ' immobilizer'],
                'products': [],
                'timeout_seconds': 30,
                'retryable': True,
            },
            {
                'name': 'submit_enrollment',
                'step_type': 'async',
                'order': 2,
                'api_endpoint': '/api/oem/maruti/enroll',
                'capabilities': [],
                'products': ['tracking', 'diagnostics', 'geofencing'],
                'timeout_seconds': 60,
                'retryable': True,
            },
            {
                'name': 'register_ready',
                'step_type': 'sync',
                'order': 3,
                'api_endpoint': '/api/oem/maruti/register-ready',
                'capabilities': [],
                'products': [],
                'timeout_seconds': 30,
                'retryable': False,
            },
        ]

        # Create steps
        for step_data in steps_data:
            step, created = WorkflowStep.objects.get_or_create(
                workflow=workflow,
                name=step_data['name'],
                defaults=step_data
            )
            if created:
                self.stdout.write(f"  Created step: {step.name}")

        # Create sequences (DAG edges)
        sequences_data = [
            ('validate_vin', 'check_capabilities', 'always'),
            ('check_capabilities', 'submit_enrollment', 'always'),
            ('submit_enrollment', 'register_ready', 'on_success'),
        ]

        for from_step, to_step, condition in sequences_data:
            seq, created = WorkflowSequence.objects.get_or_create(
                workflow=workflow,
                from_step=from_step,
                to_step=to_step,
                defaults={'condition': condition}
            )
            if created:
                self.stdout.write(f"  Created sequence: {from_step} -> {to_step}")

        self.stdout.write(self.style.SUCCESS(
            f'\nMaruti workflow created successfully!\n'
            f'  Brand: {workflow.brand}\n'
            f'  Steps: {WorkflowStep.objects.filter(workflow=workflow).count()}\n'
            f'  Sequences: {WorkflowSequence.objects.filter(workflow=workflow).count()}'
        ))

        # Print workflow summary
        self.stdout.write('\nWorkflow Steps:')
        for step in WorkflowStep.objects.filter(workflow=workflow).order_by('order'):
            self.stdout.write(f"  {step.order}. {step.name} ({step.step_type})")

        self.stdout.write('\nWorkflow Sequences:')
        for seq in WorkflowSequence.objects.filter(workflow=workflow).order_by('order'):
            self.stdout.write(f"  {seq.from_step} -> {seq.to_step} ({seq.condition})")